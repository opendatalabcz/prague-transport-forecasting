### Analýza a predikce dopravního zatížení v Praze
Zadání práce:  
Doprava v Praze představuje komplexní systém, jehož zatížení se v čase mění vlivem řady faktorů, jako jsou počasí, denní doba, den v týdnu či mimořádné události. Cílem této práce je zkoumat konkrétní vliv těchto faktorů a také vztahy mezi jednotlivými dopravními módy (MHD, cyklistická doprava, automobilová doprava). Z výsledků zkoumání poté vytvořit predikční model pro odhad zatížení těchto módů na základě historických dat.  
Cíle a dílčí úkoly práce:
1) Ověřit dostupnost a získat data o dopravě v Praze za období 2021–2025, zejména:
    - počty cestujících v MHD,
    - údaje z automatických cyklistických sčítačů,
    - data o intenzitě automobilové dopravy,
    - doplňková data o počasí a kalendářních vlivech.
2) Provedení explorační analýzy získaných dat a vizualizace zajímavých trendů.
3) Vyhodnocení kvality a použitelnosti jednotlivých datových zdrojů.
4) Návrh a implementace predikčního modelu pro odhad zatížení vybraných dopravních módů.
5) Prezentace výsledků formou interaktivního report umožňující zobrazení historických i predikovaných dat.

Doporučené reference:  
Dopravní podnik hl. m. Prahy: [online](https://www.dpp.cz/spolecnost/o-spolecnosti/zadosti-o-povinne-poskytovane-informace-dle-zakona-c-106-1999-sb/odpovedi-na-zadosti-o-informace-dle-zakona-c-106-1999-sb-v-roce-2025)  
Pražská datová platforma Golemio [online](https://api.golemio.cz/docs/openapi/).   
Ředitelství silnic a dálnic [online](https://mobilitydata.rsd.cz/).   
Český hydrometeorologický ústav: Meteorologická data [online](https://www.chmi.cz/).  
Han, J., Kamber, M., Pei, J. (2022). Data Mining: Concepts and Techniques. Morgan Kaufmann.  
Géron, A. (2023). Hands-On Machine Learning with Scikit-Learn, Keras & TensorFlow. O’Reilly Media.  
Lundberg, S. M., Lee, S.-I. (2017). A Unified Approach to Interpreting Model Predictions (SHAP). Advances in Neural Information Processing Systems.  
